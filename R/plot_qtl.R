...
#'
#' @param drop if \code{FALSE}, shows the names of all traits from \code{pheno.col}, even of those with no QTLs; if \code{TRUE} (the default), shows only the traits with QTL(s).
#'
...
plot_qtl <- function(data=data, model=model, fitted=fitted, pheno.col=NULL, main=NULL, drop=FALSE) {
  trt <- c()
  lgs <- c()
  pos <- c()
  pvl <- c()
  ...
  DF <- data.frame(Trait=trt, LG=as.integer(lgs), Pos=pos, Pval=as.numeric(pvl), H2=h2q)
  ...
  ggplot(DF, aes(x = Pos, y = Trait, colour = Pval, size = H2)) +
    ...
    theme(axis.title=element_text(face="bold"), plot.title = element_text(face="bold", hjust = 0.5), axis.ticks.x=element_blank(),
          axis.text.x = element_text(size=10), panel.grid.major = element_blank(), panel.grid.minor.x = element_line(size=1))
